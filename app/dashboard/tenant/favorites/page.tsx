...
export default function TenantFavoritesPage() {
  ...
  const removeMutation = useMutation({
    mutationFn: (propertyId: string) => removeFavorite(propertyId, session!.user!.id),
    onMutate: async (propertyId) => {
      await queryClient.cancelQueries({ queryKey: ["favorites", session?.user?.id] });
      const previous = queryClient.getQueryData<FavoriteProperty[]>(["favorites", session?.user?.id]);
      queryClient.setQueryData<FavoriteProperty[]>(["favorites", session?.user?.id], (old) =>
        old?.filter((fav) => fav._id !== propertyId) || []
      );
      return { previous };
    },
    onError: (_err, _propertyId, context) => {
      if (context?.previous) {
        queryClient.setQueryData(["favorites", session?.user?.id], context.previous);
      }
